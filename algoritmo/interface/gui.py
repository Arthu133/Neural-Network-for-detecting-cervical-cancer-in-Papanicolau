...
class CancerDetectionApp:
    def __init__(self, root):
        ...
        process_menu.add_command(label="Classify Sub-image Resnet50 Binary", command=self.classify_sub_image_bin)
        process_menu.add_command(label="Classify Sub-image SVM", command=self.classify_sub_imageSVM)
        process_menu.add_command(label="Classify Sub-image SVM Binary", command=self.classify_sub_imageSVM_bin)
    ...
    def classify_sub_image(self, event=None):
        ...
        self.model = load_model('modelo_resnet50.h5', compile=False)
        #self.model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    ...
    def classify_sub_image_bin(self, event=None):
        if self.image is None:
            messagebox.showerror("Erro", "Nenhuma imagem carregada.")
            return
        ...
        # Carregando o modelo salvo (substitua 'path_to_your_model.h5' pelo caminho correto)
        self.model = load_model('modelo_resnet50_binary.h5', compile=False)
        #self.model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    ...
    def classify_sub_imageSVM_bin(self, event=None):
        if self.image is None:
            messagebox.showerror("Erro", "Nenhuma imagem carregada.")
            return

        # Redimensionando a imagem para 16x8 pixels
        resized_image = cv2.resize(self.image, (16, 8))
        ...
        try:
            # Carregando o modelo SVM previamente treinado com joblib
            model = joblib.load('svm_binary_model.pkl')

            # Realizando a previsão usando o modelo SVM
            prediction = model.predict([feature_vector])[0]

            # Lista de rótulos de classe correspondentes às classes esperadas pelo modelo
            class_labels = ["Negative", "Others"]
